...
public class MonitorDAO extends DBConnection {
    ...
    public void update(Monitor a) {
        try {
            Statement st = this.getConnection().createStatement();
            String query = "update monitor set kampanya_id = '" + a.getKampanya().getKampanya_id() + "', boyut = " + a.getBoyut() + ", ekran_yenileme_hizi= " + a.getEkran_yenileme_hizi() + ", marka = '" + a.getMarka() + "', fiyat = " + a.getFiyat() + ", stok = " + a.getStok() + "where urun_id = " + a.getUrun_id();
            st.executeUpdate(query);
    ...
    public List<Monitor> getList() {
        ...
        try {
            Statement st = this.getConnection().createStatement();
            String query = "select * from monitor order by urun_id asc";
    ...
    public List<Monitor> getList(int page) {
        ...
        try {
            Statement st = this.getConnection().createStatement();
            ResultSet rs = st.executeQuery("select * from monitor order by urun_id asc limit 5 offset " + offset);
